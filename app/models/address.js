...
Address.updateAddress = (user_id,address_one,address_two,pincode,city_id,state_id,country_id,latitude,longitude,address_id, result) => {
  sql.query(
    "UPDATE addresses SET user_id = ?, address_one = ?, address_two = ?, pincode = ?, city = ?, state = ?, country = ?, latitude = ?, longitude = ? WHERE id = ?",
    [user_id, address_one, address_two, pincode ,city_id ,state_id ,country_id ,latitude , longitude ,address_id],
    (err, res) => {
...
Address.get_address = (user_id, result) => {
    sql.query(`SELECT a.id,a.user_id, a.address_one, a.address_two, a.city,a.state,a.country, a.latitude, a.longitude, a.created_at,
    if((SELECT id FROM buyers WHERE address_id=a.id AND user_id= ${user_id})>0,1,0) as is_default_address,a.pincode
    FROM addresses as a 
    LEFT JOIN buyers AS u ON u.id = a.user_id
