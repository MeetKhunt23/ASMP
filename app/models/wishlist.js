...
const Wishlist = function(wishlist) {
  ...
  this.seller_id = wishlist.seller_id;
...
Wishlist.isUserExist = (user_id, result) => {
    sql.query(`SELECT id FROM buyers WHERE id = ${user_id}`, (err, res) => {
    result(null, res[0]);
    return;  
    });
}
...
Wishlist.review_listing = (product_id, result) => {
    sql.query(`SELECT r.id,r.user_id,r.review,r.rating,r.created_at as posted_at,u.name as full_name
    FROM product_review as r 
    LEFT JOIN buyers AS u ON u.id = r.user_id
...
Wishlist.get_seller_review_count = (sellers_id, result) => {
    sql.query(`SELECT SUM(rating) as total_rating, COUNT(id) as total
    FROM product_review as r 
    WHERE r.seller_id = ${sellers_id}`, 
    (err, res) => {
    result(null, res[0]);
    return;
    });
